...
Correlations between blackouts and median income per census block are calculated based on the overlap of blackout locations and median income per census block provided by the U.S. Census Bureau's ACS<sup>4<sup> . Additional data access information can be found in the "Data Access" section of the README in this project repository.
...
```
  *The figure above shows luminance in (units) in Houston, TX before and after Winter storm Uri. Note multiple patches of darkness in NW and NE Houston.*
...
tm_shape(blackout_blocks) +
  tm_polygons(col = "#C0369D", alpha = 0.7) +
